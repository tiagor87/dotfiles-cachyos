...
- **Claude Code** (`claude`) — a função `c` (`dev/claude/claude.zsh`) roda na pasta atual, em YOLO, através do Headroom. `c [args...]` · `CLAUDE_MODEL=... c` (troca o modelo) · pra rodar **sem** o Headroom use o `claude` direto — a função não tem desvio. O modelo é fixado em `claude-opus-5` porque o `--1m` do Headroom escreve `ANTHROPIC_MODEL=<opus>[1m]` e o `<opus>` default dele é uma geração atrás — **precisa de bump a cada modelo novo**. `CLAUDE.md` global versionado em `dev/claude/`
- **Headroom** (`headroom-ai[proxy,relevance,code]`, via `uv tool`) — compressão de contexto p/ o Claude Code, via `headroom wrap claude --rtk --1m`. **Os extras não são opcionais**: sem `proxy` o `c` morre em `No module named 'httpx'` ("Proxy exited with code 1", fatal) e sem `relevance` o memory sync morre em `No module named 'numpy'`; `code` é o tree-sitter do `--code-graph`. Também **não** uso o `[all]` que o upstream documenta — ele arrasta wheels de torch/nvidia (venv de 5,8 GB medido aqui) pra features que o `c` não usa. **Não** dá pra trocar o `wrap` por `ANTHROPIC_BASE_URL`: com a base URL apontando pra um host customizado, o Claude Code aplica um gate client-side que desliga a janela de 1M tokens (headroom#1158), o carregamento de tools sob demanda (headroom#746) e o `/remote-control`
- **RTK** ([rtk-ai/rtk](https://github.com/rtk-ai/rtk)) — camada **complementar** ao Headroom, não concorrente. O Headroom comprime o contexto depois que os tokens já estão na request e trava no `prefix_frozen` (não comprime se isso invalidar o prefixo do prompt cache); o rtk filtra a saída dos comandos de shell **antes** dela virar request, onde não há prefixo pra invalidar. **Não precisa instalar**: o `headroom wrap` baixa o binário pra `~/.headroom/bin/rtk`, symlinka em `~/.local/bin/` e registra o hook `PreToolUse` sozinho — nada de `rtk-bin` do AUR, que só criaria um segundo binário disputando o PATH. Mas isso **só acontece com `--rtk`**: o rtk é opt-in no headroom (`_rtk_opt_in()` em `headroom/cli/wrap.py`) e sem a flag o setup inteiro é barrado calado — nem binário, nem hook, nem `~/.claude/RTK.md`, enquanto o `dev/claude/CLAUDE.md` versionado seguia documentando o hook como ativo e importando um `@RTK.md` inexistente. Economia real: `rtk gain`. Escape por comando: `rtk proxy <cmd>`
...
├── dev/                          # categoria Dev
│   ├── install/                  # 1-jetbrains-toolbox..4-runtimes 5-claude-code 7-headroom 8-claude-hud 9-beekeeper-studio 10-headroom-wrappers 11-posting 12-herdr 13-zed
