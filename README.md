...
| Desktop | 11 | `desktop/install/11-face-unlock.sh` | **Reconhecimento facial por infravermelho** (`howdy-next`), se houver câmera IR: detecta o nó v4l2 pelo sysfs, baixa os modelos, aponta o `device_path` para o nó IR, **checa o bloqueio de hardware da câmera** (controle V4L2 `privacy`, a tecla de câmera do teclado / Fn+F10: é read-only, não dá pra destravar por software — bloqueada, a câmera entrega quadros de valor **constante**, o que enganava o teste do emissor), **mede se o emissor IR acende** (captura 15 quadros e olha média **e amplitude** — sem emissor o Howdy só vê preto; amplitude 0 é preenchimento, não imagem), ajusta o `max_height` para a resolução nativa do sensor, **cadastra vários modelos em ciclo** (um por condição — sem óculos, de óculos, luz diferente; o Howdy compara contra todos, então é ganho sem afrouxar limiar) testando cada um na hora, e **valida com `howdy test`**. **Por default para aí — não toca em nenhum stack de PAM.** A integração é opt-in e exige o `howdy test` verde: `DOTFILES_FACE_PAM=1` (sudo + `/etc/pam.d/dankshell-face` p/ o lock) e `DOTFILES_FACE_GREETER=1` (greetd/login). Cada arquivo tocado ganha `.bak-dotfiles-face`. No lock o rosto **não é hands-free**: o DMS só inicia o PAM no Enter. Sem câmera IR, sai sem alterar nada |
...
| Dev | 2 | `dev/install/2-docker-desktop.sh` | Instala o **Docker Desktop** (AUR), **corrige o login** (gera chave GPG + `pass init` — o credential helper do Docker no Linux usa `pass`; sem isso o Sign in não persiste) e **limita os recursos da VM** (`Cpus=4`, `MemoryMiB=4096`, `DiskSizeMiB=131072`): sem isso a VM QEMU sobe com `-smp <todos os cores>` e disputa CPU com o desktop. Também **expõe o socket no caminho padrão** (`/run/docker.sock` → `~/.docker/desktop/docker.sock`, via `tmpfiles.d`), senão tudo que ignora o contexto do docker (Dev Containers, Testcontainers, IDEs) quebra com *"Cannot connect to the Docker daemon at unix:///var/run/docker.sock"* |
...
| Dev | 5 | `dev/install/5-claude-code.sh` | Instala o **Claude Code** (+ jq), liga o `CLAUDE.md` global e as skills do repo, e as funções `c`/`bc` ao `.zshrc`. Se já houver config em `~/.claude`, **pergunta se é pra fazer um setup limpo** (zera `settings.json`, plugins e hooks — nunca o login nem o `projects/`) |
| Dev | 6 | `dev/install/6-ai-memory.sh` | Instala o **ai-memory** (AUR `ai-memory-bin`), roda o `init`, habilita o serviço de usuário (loopback `:49374`) e fia o MCP + os 9 hooks de ciclo de vida **nos dois perfis** do Claude Code (`~/.claude` do `c` e `~/.claude-bedrock` do `bc`). Roda **depois** do 5 de propósito: o setup limpo de lá zera o `settings.json`, que é onde os hooks moram |
| Dev | 7 | `dev/install/7-headroom.sh` | Instala o **Headroom** (compressão de contexto, via `uv tool`). A integração hoje é na função `codex`, que chama `headroom wrap codex` — o `wrap` sobe/reaproveita o proxy sozinho. O Claude Code saiu do wrap no `8e56f05` |
...
- **Docker Desktop** (AUR) — autostart no login (serviço de usuário); login corrigido via `pass`/GPG. **VM limitada** a 4 vCPUs / 4 GiB RAM / 128 GiB de teto de disco em `~/.docker/desktop/settings-store.json` (vale após `docker desktop restart`). Ajustável por env: `DD_CPUS`, `DD_MEMORY_MIB`, `DD_DISK_MIB`. Reduzir o teto de disco pede confirmação — pode fazer o Docker recriar o `Docker.raw`
  - **Socket padrão**: no Linux o Docker Desktop só escuta em `~/.docker/desktop/docker.sock` (contexto `desktop-linux`) e a opção *"Allow the default Docker socket"* da UI não existe aqui — o pacote não traz helper privilegiado. O `/etc/tmpfiles.d/docker-desktop-socket.conf` linka `/run/docker.sock` (= `/var/run/docker.sock`) pro socket do Desktop, e o systemd recria o link em cada boot (`/run` é tmpfs). Se algum dia o **Docker CE** (`docker.service`/`docker.socket`) for habilitado, o script pula esse passo — o socket padrão passa a ser dele
...
- **Claude Code** (`claude`) — a função `c` (`dev/claude/claude.zsh`) roda na pasta atual, em YOLO, através do `ai-memory run` (workstream gerenciado: hooks de sessão gravam as observações, o MCP `ai-memory` devolve a memória). O `--yolo` é flag **do wrapper**, não do claude. Até o commit `8e56f05` isso era `headroom wrap claude --rtk --1m` — saiu junto com o wrap a janela de 1M por flag (hoje se escolhe no `/model`, e o `CLAUDE_MODEL` fixo deixou de ter leitor) e o hook do rtk. O `ai-memory` vem do `dev/install/6-ai-memory.sh` — sem ele no PATH, `c` e `bc` morrem em *command not found*; pra rodar sem wrapper, `claude` direto. Ressalva: o `c` **não repassa args** (`c --resume` é engolido), o `bc` repassa. `CLAUDE.md` global versionado em `dev/claude/`
- **ai-memory** ([akitaonrails/ai-memory](https://github.com/akitaonrails/ai-memory), AUR `ai-memory-bin`) — memória de longo prazo dos agentes: os **hooks de ciclo de vida** gravam observações sanitizadas enquanto a sessão roda, o **servidor MCP** (loopback `:49374`, serviço de usuário do systemd) devolve isso pro agente na sessão seguinte. É dependência dura do `c`/`bc`. O `-bin` traz o binário pré-compilado das releases do próprio autor; o pacote `ai-memory` compila do fonte em Rust, sem ganho aqui. `ai-memory status` · `ai-memory recent` · `systemctl --user status ai-memory`
- **Claude Code no Bedrock** — a função `bc` (mesmo `dev/claude/claude.zsh`) sobe o Claude Code contra o Amazon Bedrock, com config **isolada em `~/.claude-bedrock`** (`CLAUDE_CONFIG_DIR`) pra não misturar credencial, MCPs e histórico com os do `c`. Os identificadores **não moram no repo** (o ARN do inference profile carrega o número da conta AWS, e isto aqui é público): vêm do `~/.zshenv`, uma ARN por modelo (`BEDROCK_ARN_HAIKU_4_5` / `_SONNET_4_6` / `_SONNET_5` / `_OPUS_4_8` / `_OPUS_5`, + `BEDROCK_GUARDRAIL_ID`/`_VERSION`, opcionais). Sem as cinco, `bc` recusa a subir com a lista do que falta. A cada execução, `bc` escreve (via `jq`, preservando o resto do arquivo — os hooks do `ai-memory`) `model` / `availableModels` / `enforceAvailableModels` / `modelOverrides` no `settings.json` do perfil: é o `modelOverrides` que resolve um agente autônomo/subagent escolhendo um modelo fora do trio default (`ANTHROPIC_DEFAULT_*_MODEL` só cobre esse trio) — sem ele, o alias escolhido ia literal pro Bedrock e era rejeitado. A credencial vem do AWS CLI do ambiente, perfil **`bloquo-bedrock`** por default (role dedicada, região `us-east-1` — não existe perfil `default` neste `~/.aws/config`); um preflight com `timeout 8` confere a credencial antes de entrar no `ai-memory run`, pra falhar em segundos com mensagem clara em vez de ficar pendurado tentando IMDS/ECS numa máquina que não é EC2. `bc [args...]` · `AWS_PROFILE=... bc` (troca o perfil) · `AWS_REGION=... bc` · `CLAUDE_BEDROCK_DIR=... bc`
- **Headroom** (`headroom-ai[proxy,relevance,code]`, via `uv tool`) — compressão de contexto. **Hoje quem usa é o `codex`** (`dev/codex/codex.zsh`, via `headroom wrap codex`); o Claude Code saiu do wrap no `8e56f05`. **Os extras não são opcionais**: sem `proxy` o wrap morre em `No module named 'httpx'` ("Proxy exited with code 1", fatal) e sem `relevance` o memory sync morre em `No module named 'numpy'`; `code` é o tree-sitter do `--code-graph`. Também **não** uso o `[all]` que o upstream documenta — ele arrasta wheels de torch/nvidia (venv de 5,8 GB medido aqui) pra features não usadas. Registro do porquê o wrap existia em vez de `ANTHROPIC_BASE_URL`: com a base URL apontando pra um host customizado, o Claude Code aplica um gate client-side que desliga a janela de 1M tokens (headroom#1158), o carregamento de tools sob demanda (headroom#746) e o `/remote-control`
- **RTK** ([rtk-ai/rtk](https://github.com/rtk-ai/rtk)) — filtrava a saída dos comandos de shell **antes** dela virar request (`git diff` com menos contexto, `pytest` só com as falhas, `cat` com assinaturas), via hook `PreToolUse`. **Está desligado**: quem instalava o hook era o `headroom wrap ... --rtk`, e o `c` não passa mais por ele — conferido, o `~/.claude/settings.json` não tem `hooks` e o `~/.claude/RTK.md` não existe. O binário segue em `~/.local/bin/rtk` de instalações anteriores e dá pra usar à mão (`rtk proxy <cmd>`, `rtk gain`). **Consequência**: a saída do Bash não é mais truncada por design — o que o `dev/claude/CLAUDE.md` afirmava e deixou de ser verdade
...
- **Zed** (`zed`) — editor GPU: o pacote do Arch instala o binário como **`zeditor`**, então o setup linka `~/.local/bin/zed → /usr/bin/zeditor` (symlink, não alias — vale em scripts, no `EDITOR` e no git) pra abrir com **`zed <path>`**. `settings.json` versionado em `dev/zed/`, com JetBrainsMono Nerd Font e as cores **Material You** do DMS (template `Zed` do matugen → `~/.config/zed/themes/dank-zed-theme.json`, temas `DankShell Dark`/`DankShell Light`) que acompanham o wallpaper. O tema só existe depois da primeira regeração de cores (troca de wallpaper ou `dms ipc call theme toggle`); antes disso o Zed cai no One Dark. Telemetria desligada. Resto nos **defaults** — o Zed reescreve esse arquivo quando se muda algo pela UI
